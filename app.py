...
# =========================================================
# 1. CONFIGURACIÓN INICIAL (Texto exacto del usuario)
# =========================================================
st.set_page_config(
...
# --- MODELO (Lógica robusta del Segundo Código) ---
class MockModel:
    def predict_proba(self, X):
        # Simulación simple si no hay modelo real
        if isinstance(X, pd.DataFrame):
            # Pesos simulados
            val_glucose = X.iloc[0].get('Glucose', 100)
            val_bmi = X.iloc[0].get('BMI', 25)
            val_age = X.iloc[0].get('Age', 30)
            score = (val_glucose*0.5) + (val_bmi*0.4) + (val_age*0.1) 
        else:
            score = 50
...
# =========================================================
# 3. ESTILOS CSS (Texto exacto y diseño del usuario)
# =========================================================
CEMP_PINK = "#E97F87"
CEMP_DARK = "#2C3E50"
GOOD_TEAL = "#4DB6AC"
...
# ==========================================
# 5. NAVEGACIÓN Y PÁGINAS (LOGICA DEL USUARIO)
# ==========================================
...
if st.session_state.step > 1:
    ...
    # --- RENDERIZADO DE LA BARRA LATERAL (TEXTO EXACTO) ---
    with st.sidebar:
...
if st.session_state.step == 3:
    ...
    # Se añade un valor dummy para Presión Arterial que no estaba en el input del texto original pero el modelo lo necesita
    # Mapeo: 'Pregnancies', 'Glucose', 'BloodPressure', 'Insulin', 'BMI', 'DPF', 'Age', 'Indice_resistencia', 'BMI_square', 'Is_prediabetes'
    is_prediabetes = 1 if glucose >= 140 else 0
    input_data = pd.DataFrame([[
        pregnancies, glucose, 70, insulin, bmi, dpf, age, proxy_index, bmi_sq, is_prediabetes
    ...
    with tab2:
        ...
        if SHAP_AVAILABLE and hasattr(st.session_state.model, 'named_steps'):
            try:
                ...
                plt.tight_layout()
                chart_html_shap = fig_to_html(fig_shap); plt.close(fig_shap)
                st.markdown(f"""<div class="card"><h3 style="color:{CEMP_DARK}; font-size:1.2rem; margin-bottom:5px;">Factores de Riesgo (SHAP)</h3><span class="card-header" style="margin-bottom:20px;">EXPLICABILIDAD DEL MODELO</span>{chart_html_shap}</div>""", unsafe_allow_html=True)
            except Exception as e:
                 st.error(f"Error generando SHAP: {e}")
        else:
            st.warning("Librería SHAP no disponible o modelo simulado. (Se requiere modelo real cargado)")
